...
export default (diems, { startDate, endDate, activityText }) => {
  return diems
    .filter((diem) => {
      ...
      const activityTextMatch = diem.activities
        .map(({ name }) => name.toLowerCase().includes(activityText.toLowerCase()))
        .includes(true);
      const remainderMatch = diem.remainder
        ? 'unrecorded'.includes(activityText.toLowerCase())
        : false;
